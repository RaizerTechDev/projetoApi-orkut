...
app.post("/usuarios", validarUsuarios, async (req, res) => {
  try {
    const { nome, email, senha } = req.body; //Atualizar corpo da requisição

    // Criptografar a senha usando bcrypt com um salt de 10 rounds pra quantas vezes será aplicado, assim, a senha fica mais segura e difícil de ser quebrada por ataques de força bruta. O resultado é uma string hash que pode ser armazenada no banco de dados. 
...
app.get("/usuarios", async (req, res) => {
  try {
    ...
    post: (resultado.rows[0], res.json(resultado.rows));
  } catch (erro) {
